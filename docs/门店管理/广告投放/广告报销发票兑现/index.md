...
<pre class="detail-sql" v-pre><code>SELECT c.fee_cashout_id      AS 兑现单ID,
...
<pre class="detail-sql" v-pre><code>SELECT confname            AS 配置名称,
         confvalue           AS 配置值
  FROM   scpsysconf
  WHERE  confname = 'Outlimit_Tax_Rate';</code></pre>
