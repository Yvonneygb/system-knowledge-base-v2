...
<div class="kl-card">
  <div class="biz-kl-hdr">
    <span class="biz-tag" style="background:rgba(124,58,237,0.08);color:#7C3AED;border-color:rgba(124,58,237,0.18);"> 重点逻辑1</span>
    <h2>LOV 值集配置</h2>
    <p>装修等级是门店管理和财务报销的核心基础数据</p>
  </div>
  <ul>
    <li><strong>业务意义</strong>：装修等级是门店管理和财务报销的核心基础数据，不同装修等级对应不同的报销标准，需统一配置和维护。</li>
    <li><strong>具体逻辑描述</strong>：通过 HZERO 平台标准 LOV 配置界面维护值集 <code>AE.FIXUP_GRADE</code>，每个装修等级对应一个 <code>VALUE</code>（值编码）和 <code>MEANING</code>（值名称）。所有业务表中 <code>FIXUP_GRADE</code> 字段统一存储 <code>VALUE</code>（Long 类型数值），界面展示时通过 <code>lovService.getMeaning("AE.FIXUP_GRADE", value)</code> 翻译为 <code>MEANING</code>（中文含义）。</li>
  </ul>
</div>

<div class="kl-card">
  <div class="biz-kl-hdr">
    <span class="biz-tag" style="background:rgba(124,58,237,0.08);color:#7C3AED;border-color:rgba(124,58,237,0.18);"> 重点逻辑2</span>
    <h2>值编码唯一性</h2>
    <p>确保同一 LOV 下值编码不重复，避免业务引用歧义</p>
  </div>
  <ul>
    <li><strong>业务意义</strong>：确保同一 LOV 下值编码不重复，避免业务引用歧义。</li>
    <li><strong>具体逻辑描述</strong>：同一 <code>LOV_CODE = 'AE.FIXUP_GRADE'</code> 下 <code>VALUE</code> 字段唯一。新增/编辑保存时由 HZERO 平台校验，若重复提示"等级编码已存在"。</li>
  </ul>
</div>

<div class="kl-card">
  <div class="biz-kl-hdr">
    <span class="biz-tag" style="background:rgba(124,58,237,0.08);color:#7C3AED;border-color:rgba(124,58,237,0.18);"> 重点逻辑3</span>
    <h2>生效日期控制</h2>
    <p>值项可通过开始/结束日期控制生效范围</p>
  </div>
  <ul>
    <li><strong>业务意义</strong>：支持装修等级的版本化管理。</li>
    <li><strong>具体逻辑描述</strong>：值项配置 <code>START_DATE</code> 和 <code>END_DATE</code>，业务查询时仅返回当前日期在生效范围内的值项，过期或未生效的值项不在下拉列表中展示。</li>
  </ul>
</div>

<div class="kl-card">
  <div class="biz-kl-hdr">
    <span class="biz-tag" style="background:rgba(124,58,237,0.08);color:#7C3AED;border-color:rgba(124,58,237,0.18);"> 重点逻辑4</span>
    <h2>排序号控制</h2>
    <p>控制值项在下拉列表中的展示顺序，提升用户选择体验</p>
  </div>
  <ul>
    <li><strong>业务意义</strong>：控制值项在下拉列表中的展示顺序，提升用户选择体验。</li>
    <li><strong>具体逻辑描述</strong>：通过 <code>ORDER_SEQ</code> 字段控制值项展示顺序，查询时按 <code>ORDER_SEQ</code> 升序排列。</li>
  </ul>
</div>

<div class="kl-card">
  <div class="biz-kl-hdr">
    <span class="biz-tag" style="background:rgba(124,58,237,0.08);color:#7C3AED;border-color:rgba(124,58,237,0.18);"> 重点逻辑5</span>
    <h2>多事业部适配</h2>
    <p>支持不同事业部自定义装修等级的值和含义</p>
  </div>
  <ul>
    <li><strong>业务意义</strong>：支持不同事业部自定义装修等级的值和含义。</li>
    <li><strong>具体逻辑描述</strong>：当前所有事业部共用同一套 <code>VALUE → MEANING</code> 映射，通过 <code>TAG</code> 字段区分适用的事业部（<code>organization_id</code>）。如需支持不同事业部自定义，需在 Java 翻译层增加 <code>organizationId</code> 参数，并调整值集配置方式。</li>
  </ul>
</div>
...
<div class="kl-card">
  <div class="biz-kl-hdr">
    <span class="biz-tag" style="background:rgba(124,58,237,0.08);color:#7C3AED;border-color:rgba(124,58,237,0.18);"> 界面模块</span>
    <h2>查询栏字段</h2>
  ...
  <table class="kl-table">
    ...
    <tr><td>LOV 编码</td><td>LOV_CODE</td><td>TextField</td><td>LOV 编码</td><td>始终显示</td><td>固定为 <code>AE.FIXUP_GRADE</code>，不可编辑</td></tr>
...
<div class="kl-card">
  <div class="biz-kl-hdr">
    <span class="biz-tag" style="background:rgba(124,58,237,0.08);color:#7C3AED;border-color:rgba(124,58,237,0.18);"> 列表展示</span>
    <h2>列表展示字段</h2>
    <p>装修等级值项列表展示的字段</p>
  </div>
  <table class="kl-table">
...
<div class="kl-card">
  <div class="biz-kl-hdr">
    <span class="biz-tag" style="background:rgba(124,58,237,0.08);color:#7C3AED;border-color:rgba(124,58,237,0.18);"> 按钮操作</span>
    <h2>其他按钮</h2>
    <p>列表工具栏与行操作按钮</p>
  </div>
  <table class="kl-table">
...
<div class="kl-card">
  <div class="biz-kl-hdr">
    <span class="biz-tag" style="background:rgba(124,58,237,0.08);color:#7C3AED;border-color:rgba(124,58,237,0.18);"> 按钮逻辑</span>
    <h2>按钮执行逻辑</h2>
    <p>各按钮的触发条件与执行逻辑</p>
  </div>

  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:0 0 8px;">按钮1：新建（列表工具栏）</h4>
  <p style="font-size:0.8rem;color:#374151;line-height:1.7;margin:0 0 6px;"><strong>触发条件</strong>：点击"新建"按钮</p>
  <p style="font-size:0.8rem;color:#374151;line-height:1.7;margin:0 0 12px;"><strong>执行逻辑</strong>：</p>
  <ol style="font-size:0.8rem;color:#374151;margin:0 0 16px;">
    <li>打开新建值项弹窗/抽屉</li>
    <li>填写值编码、值名称、描述、排序号、开始/结束日期</li>
    <li>保存时校验值编码唯一性、必填项</li>
    <li>调用 HZERO 平台 LOV 服务新增接口写入 <code>HPFM_LOV_VALUE</code> 表</li>
  </ol>

  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:0 0 8px;">按钮2：编辑（列表行操作）</h4>
  <p style="font-size:0.8rem;color:#374151;line-height:1.7;margin:0 0 6px;"><strong>触发条件</strong>：选择一条数据点击"编辑"</p>
  <p style="font-size:0.8rem;color:#374151;line-height:1.7;margin:0 0 12px;"><strong>执行逻辑</strong>：</p>
  <ol style="font-size:0.8rem;color:#374151;margin:0 0 16px;">
    <li>打开编辑值项弹窗/抽屉，回显当前值项数据</li>
    <li>修改值项信息（值编码通常不可编辑）</li>
    <li>保存时校验必填项</li>
    <li>调用 HZERO 平台 LOV 服务更新接口更新 <code>HPFM_LOV_VALUE</code> 表</li>
  </ol>

  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:0 0 8px;">按钮3：删除（列表行操作）</h4>
  <p style="font-size:0.8rem;color:#374151;line-height:1.7;margin:0 0 6px;"><strong>触发条件</strong>：选择一条数据点击"删除"</p>
  <p style="font-size:0.8rem;color:#374151;line-height:1.7;margin:0 0 12px;"><strong>执行逻辑</strong>：</p>
  <ol style="font-size:0.8rem;color:#374151;margin:0 0 16px;">
    <li>弹出确认框："确定删除该装修等级吗？"</li>
    <li>确认后调用 HZERO 平台 LOV 服务删除接口</li>
    <li>检查该值项是否被业务数据引用，若被引用则提示无法删除</li>
  </ol>

  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:0 0 8px;">按钮4：启用/禁用（列表行操作）</h4>
  <p style="font-size:0.8rem;color:#374151;line-height:1.7;margin:0 0 6px;"><strong>触发条件</strong>：选择一条数据点击"启用/禁用"</p>
  <p style="font-size:0.8rem;color:#374151;line-height:1.7;margin:0 0 12px;"><strong>执行逻辑</strong>：</p>
  <ol style="font-size:0.8rem;color:#374151;margin:0 0 16px;">
    <li>切换 <code>ENABLE_FLAG</code> 字段值（1↔0）</li>
    <li>调用 HZERO 平台 LOV 服务更新接口</li>
    <li>禁用后该值项不在业务下拉列表中展示</li>
  </ol>
</div>

<div class="kl-card">
  <div class="biz-kl-hdr">
    <span class="biz-tag" style="background:rgba(124,58,237,0.08);color:#7C3AED;border-color:rgba(124,58,237,0.18);"> 校验</span>
    <h2>保存校验</h2>
    <p>保存时的校验规则</p>
  </div>
  <ul>
    <li><strong>校验1</strong>：值编码必填 —— 确保值项有唯一标识</li>
    <li><strong>校验2</strong>：值编码唯一 —— 同一 LOV 下值编码不可重复，避免业务引用歧义</li>
    <li><strong>校验3</strong>：值名称必填 —— 确保值项有展示名称</li>
    <li><strong>校验4</strong>：排序号必填 —— 确保值项在下拉列表中有明确顺序</li>
  </ul>
</div>
...
<div class="kl-card">
  <div class="biz-kl-hdr">
    <span class="biz-tag" style="background:rgba(124,58,237,0.08);color:#7C3AED;border-color:rgba(124,58,237,0.18);"> 业务引用</span>
    <h2>业务引用说明</h2>
    <p>装修等级值集 AE.FIXUP_GRADE 在多个业务模块中被引用</p>
  </div>
  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:0 0 8px;">引用菜单汇总</h4>
  <table class="kl-table">
    ...
    <tr><td>门店变更</td><td>MktTerminalModify</td><td>装修等级（变更前/后）</td><td>VALUE</td><td>门店变更属性字段</td></tr>
  ...
  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:16px 0 8px;">Java 层翻译方式</h4>
  <table class="kl-table">
    <tr><th>翻译方式</th><th>代码</th><th>方向</th><th>使用场景</th></tr>
    <tr><td>标准翻译</td><td><code>lovService.getMeaning("AE.FIXUP_GRADE", value)</code></td><td>VALUE→MEANING</td><td>打印/展示时将数值翻译为中文</td></tr>
    <tr><td>反向翻译</td><td><code>lovService.getValue("AE.FIXUP_GRADE", meaning)</code></td><td>MEANING→VALUE</td><td>OA 回写时将中文反查为数值</td></tr>
    <tr><td>注解翻译</td><td><code>@LovValue(lovCode="AE.FIXUP_GRADE", meaningField="fixupGradeMeaning")</code></td><td>VALUE→MEANING</td><td>VO 返回前端时自动翻译</td></tr>
  </table>

  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:16px 0 8px;">SQL 层翻译方式</h4>
  <table class="kl-table">
    <tr><th>翻译方式</th><th>SQL</th><th>使用场景</th></tr>
    <tr><td>JOIN 翻译</td><td><code>LEFT JOIN HPFM_LOV_VALUE hlv ON hlv.LOV_CODE='AE.FIXUP_GRADE' AND hlv.VALUE=TO_CHAR(afh.FIXUP_GRADE) AND hlv.TAG LIKE CONCAT('%',CONCAT(afh.ORGANIZATION_ID,'%'))</code></td><td>导出/报表查询</td></tr>
    <tr><td>函数翻译</td><td><code>GET_DIV_DICT_NAME('AE.FIXUP_GRADE', TO_CHAR(cbh.FIXUP_GRADE), cbh.ORGANIZATION_ID)</code></td><td>导出/报表查询</td></tr>
  </table>
</div>
...
<div class="kl-card">
  <div class="biz-kl-hdr">
    ...
    <h2>HPFM_LOV（HZERO 平台 LOV 值表）</h2>
    <p>值集定义表</p>
  </div>
  <table class="kl-table">
    <tr><th>字段名</th><th>类型</th><th>释义</th><th>对应界面字段</th><th>逻辑</th></tr>
    <tr><td>LOV_ID</td><td>Long</td><td>主键ID</td><td>无</td><td>自增</td></tr>
    <tr><td>LOV_CODE</td><td>VARCHAR</td><td>LOV 编码</td><td>LOV 编码</td><td>固定为 <code>AE.FIXUP_GRADE</code></td></tr>
...
<div class="kl-card">
  <div class="biz-kl-hdr">
    <span class="biz-tag" style="background:rgba(124,58,237,0.08);color:#7C3AED;border-color:rgba(124,58,237,0.18);"> 报错一览</span>
    <h2>报错一览表</h2>
    <p>装修等级值集配置与引用过程中可能出现的报错</p>
  </div>
  <table class="kl-table">
    ...
    <tr><td>无政策标准不能提交！</td><td>门店装修申请提交时</td><td>FIXUP_GRADE 字段为空或为 0；选择有效的装修等级</td><td>错误</td><td>提交校验 FIXUP_GRADE 非空非 0</td></tr>
...
<div class="kl-card">
  <div class="biz-kl-hdr">
    <span class="biz-tag" style="background:rgba(124,58,237,0.08);color:#7C3AED;border-color:rgba(124,58,237,0.18);"> 报错详情</span>
    <h2>报错详情弹窗</h2>
    <p>各报错的触发条件、逻辑分析与排查SQL</p>
  </div>

  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:0 0 8px;">报错1：等级编码已存在</h4>
  <ul style="font-size:0.8rem;color:#374151;margin:0 0 12px;">
    <li><strong>触发条件</strong>：新增或编辑保存装修等级值项时，同一 LOV（AE.FIXUP_GRADE）下 VALUE 字段与已有值项重复</li>
    <li><strong>逻辑分析</strong>：后端保存前查询 HPFM_LOV_VALUE 表，校验同一 LOV_CODE='AE.FIXUP_GRADE' 下 VALUE 唯一性，若存在重复则提示"等级编码已存在"。确保值编码唯一，避免业务引用歧义</li>
  </ul>
...
HAVING COUNT(*) &gt; 1;</code></pre>
  ...
  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:16px 0 8px;">报错2：值编码不能为空</h4>
  <ul style="font-size:0.8rem;color:#374151;margin:0 0 12px;">
    <li><strong>触发条件</strong>：新增或编辑保存时，VALUE 字段未填写</li>
    <li><strong>逻辑分析</strong>：前端表单对 VALUE 字段配置 required 校验，提交前校验值编码是否填写，为空则阻止提交并提示"值编码不能为空"。值编码是值项的唯一标识，业务表存储此值，必须明确</li>
  </ul>
...
WHERE LOV_CODE = 'AE.FIXUP_GRADE'
  ...
  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:16px 0 8px;">报错3：值名称不能为空</h4>
  <ul style="font-size:0.8rem;color:#374151;margin:0 0 12px;">
    <li><strong>触发条件</strong>：新增或编辑保存时，MEANING 字段未填写</li>
    <li><strong>逻辑分析</strong>：前端表单对 MEANING 字段配置 required 校验，提交前校验值名称是否填写，为空则阻止提交并提示"值名称不能为空"。值名称用于界面展示，必须明确</li>
  </ul>
  <pre class="detail-sql" v-pre><code>SELECT LOV_VALUE_ID AS 值项ID,
       LOV_CODE AS LOV编码,
       VALUE AS 值编码,
...
WHERE LOV_CODE = 'AE.FIXUP_GRADE'
  ...
  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:16px 0 8px;">报错4：店面装修等级没有对应的政策标准</h4>
  <ul style="font-size:0.8rem;color:#374151;margin:0 0 12px;">
    <li><strong>触发条件</strong>：门店装修申请提交时，所选装修等级在 TERMINAL_DECORATE_LINE 表中无对应的标准行</li>
    <li><strong>逻辑分析</strong>：后端提交校验查询 TERMINAL_DECORATE_LINE 表，根据 FIXUP_GRADE 匹配装修标准行，若无对应标准行则提示"店面装修等级没有对应的政策标准"。确保装修等级有对应的报销标准，避免提交后无法计算补贴金额</li>
  </ul>
...
WHERE lv.LOV_CODE = 'AE.FIXUP_GRADE'
  ...
  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:16px 0 8px;">报错5：无政策标准不能提交！</h4>
  <ul style="font-size:0.8rem;color:#374151;margin:0 0 12px;">
    <li><strong>触发条件</strong>：门店装修申请提交时，FIXUP_GRADE 字段为空或为 0</li>
    <li><strong>逻辑分析</strong>：后端提交校验 FIXUP_GRADE 字段非空非 0，若为空或为 0 则提示"无政策标准不能提交！"。装修等级是匹配报销标准的必要字段，必须选择有效值</li>
  </ul>
...
WHERE FIXUP_GRADE IS NULL
  ...
  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:16px 0 8px;">报错6：店面装修等级没有有效期内的政策标准！</h4>
  <ul style="font-size:0.8rem;color:#374151;margin:0 0 12px;">
    <li><strong>触发条件</strong>：门店装修申请提交时，装修等级在有效期内无对应标准行</li>
    <li><strong>逻辑分析</strong>：后端提交校验精确匹配有效期内标准行，查询 TERMINAL_DECORATE_LINE 表中 FIXUP_GRADE 匹配且 START_DATE &lt;= 当前日期 &lt;= END_DATE 的标准行，若无结果则提示"店面装修等级没有有效期内的政策标准！"。确保装修等级在当前时间有有效的报销标准</li>
  </ul>
  <pre class="detail-sql" v-pre><code>SELECT lv.VALUE AS 装修等级编码,
       lv.MEANING AS 装修等级名称,
...
WHERE lv.LOV_CODE = 'AE.FIXUP_GRADE'
  ...
  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:16px 0 8px;">报错7：值集编码不存在</h4>
  <ul style="font-size:0.8rem;color:#374151;margin:0 0 12px;">
    <li><strong>触发条件</strong>：查询装修等级值集时，HPFM_LOV 表中无 LOV_CODE='AE.FIXUP_GRADE' 的记录</li>
    <li><strong>逻辑分析</strong>：后端查询 HPFM_LOV 表，若不存在 LOV_CODE='AE.FIXUP_GRADE' 的值集定义则提示"值集编码不存在"。需先在 HZERO 值集管理中创建值集 AE.FIXUP_GRADE，再配置值项</li>
  </ul>
  <pre class="detail-sql" v-pre><code>SELECT LOV_ID, LOV_CODE, LOV_TYPE_CODE, DESCRIPTION
FROM HPFM_LOV
WHERE LOV_CODE = 'AE.FIXUP_GRADE';</code></pre>

  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:16px 0 8px;">报错8：网络异常/接口超时</h4>
  <ul style="font-size:0.8rem;color:#374151;margin:0 0 12px;">
    <li><strong>触发条件</strong>：任意接口调用时，网络中断或接口响应超过 axios timeout 配置</li>
    <li><strong>逻辑分析</strong>：前端 axios 请求未收到响应或响应超时，触发 catch 回调统一提示"请求失败"。常见根因：网络中断、hzero-platform 服务假死、数据库慢查询等。需检查网络连通性、后端服务负载、数据库性能</li>
  </ul>
...
ORDER BY LAST_UPDATE_DATE DESC;</code></pre>
  ...
  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:16px 0 8px;">报错9：权限不足</h4>
  <ul style="font-size:0.8rem;color:#374151;margin:0 0 12px;">
    <li><strong>触发条件</strong>：点击新增、编辑、删除、启用/禁用等按钮时，当前用户无对应 permissionList 权限码</li>
    <li><strong>逻辑分析</strong>：前端 Button 组件通过 permissionList 配置权限码，HZERO 框架校验当前用户角色是否包含该权限码，未包含则按钮不可见或禁用。若强制调用接口，后端也会校验权限返回403。需联系管理员配置对应角色权限</li>
  </ul>
...
WHERE P.PERMISSION_CODE LIKE '%fixup_grade%' ORDER BY U.USER_NAME;</code></pre>
  ...
  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:16px 0 8px;">报错10：数据不存在</h4>
  <ul style="font-size:0.8rem;color:#374151;margin:0 0 12px;">
    <li><strong>触发条件</strong>：编辑、删除等操作时，接口返回数据为空或值项ID不存在</li>
    <li><strong>逻辑分析</strong>：前端通过 lovValueId 调用接口，后端查询 HPFM_LOV_VALUE 表无对应记录或记录已逻辑删除，返回空数据。常见根因：值项ID错误、值项已被删除、跨租户查询、数据权限隔离等。需检查 LOV_VALUE_ID 有效性及数据权限</li>
  </ul>
...
WHERE LOV_CODE = 'AE.FIXUP_GRADE'
  ...
  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:16px 0 8px;">报错11：状态不允许操作</h4>
  <ul style="font-size:0.8rem;color:#374151;margin:0 0 12px;">
    <li><strong>触发条件</strong>：点击启用/禁用按钮时，值项状态不允许该操作</li>
    <li><strong>逻辑分析</strong>：后端校验状态机，如已启用（ENABLE_FLAG=1）不可重复启用、已禁用（ENABLE_FLAG=0）不可重复禁用等。状态不匹配时后端返回业务异常。需检查值项当前状态及操作流程</li>
  </ul>
  <pre class="detail-sql" v-pre><code>SELECT LOV_VALUE_ID AS 值项ID, VALUE AS 值编码, MEANING AS 值名称,
...
ORDER BY CREATE_DATE DESC;</code></pre>
  ...
  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:16px 0 8px;">报错12：描述不能为空</h4>
  <ul style="font-size:0.8rem;color:#374151;margin:0 0 12px;">
    <li><strong>触发条件</strong>：保存时，DESCRIPTION 字段为空</li>
    <li><strong>逻辑分析</strong>：前端表单对 DESCRIPTION 字段配置 required 校验，提交前校验描述是否填写，为空则阻止提交并提示"描述不能为空"。描述用于记录值项的业务含义，必须明确</li>
  </ul>
  <pre class="detail-sql" v-pre><code>SELECT LOV_VALUE_ID AS 值项ID, VALUE AS 值编码, MEANING AS 值名称,
       DESCRIPTION AS 描述
FROM HPFM_LOV_VALUE
WHERE LOV_CODE = 'AE.FIXUP_GRADE'
  ...
  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:16px 0 8px;">报错13：排序号不能为空</h4>
  <ul style="font-size:0.8rem;color:#374151;margin:0 0 12px;">
    <li><strong>触发条件</strong>：保存时，ORDER_SEQ 字段为空</li>
    <li><strong>逻辑分析</strong>：前端表单对 ORDER_SEQ 字段配置 required 校验，提交前校验排序号是否填写，为空则阻止提交并提示"排序号不能为空"。排序号用于值项在下拉列表中的展示顺序，必须明确</li>
  </ul>
  <pre class="detail-sql" v-pre><code>SELECT LOV_VALUE_ID AS 值项ID, VALUE AS 值编码, MEANING AS 值名称,
       ORDER_SEQ AS 排序号
FROM HPFM_LOV_VALUE
...
<div class="kl-card">
  <div class="biz-kl-hdr">
    <span class="biz-tag" style="background:rgba(124,58,237,0.08);color:#7C3AED;border-color:rgba(124,58,237,0.18);"> FAQ</span>
    <h2>常见问题FAQ</h2>
    <p>装修等级值集常见问题与排查方式</p>
  </div>
  <table class="kl-table">
    <tr><th>问题</th><th>答案/排查方式</th></tr>
    <tr><td>装修等级下拉无选项</td><td>1. 检查值集 <code>AE.FIXUP_GRADE</code> 是否创建；2. 检查值项 <code>ENABLE_FLAG</code> 是否为 1；3. 检查当前组织是否有值集数据（<code>TAG</code> 字段）</td></tr>
    <tr><td>装修等级与报销标准不匹配</td><td>1. 检查装修标准配置中是否包含该等级；2. 在财务费用申请页面提示"店面装修等级没有对应的政策标准"时补充配置</td></tr>
    <tr><td>装修等级排序不正确</td><td>1. 检查 <code>ORDER_SEQ</code> 字段值；2. 按期望顺序调整排序号</td></tr>
    <tr><td>新增装修等级后其他页面不显示</td><td>1. 检查 <code>ENABLE_FLAG</code> 是否为 1（启用）；2. 检查 <code>START_DATE</code>/<code>END_DATE</code> 生效范围；3. 清除浏览器缓存刷新页面</td></tr>
    <tr><td>OA 推送装修等级显示数值而非中文</td><td>检查 <code>lovService.getMeaning</code> 翻译是否正常，确认 <code>HPFM_LOV_VALUE</code> 中有对应 <code>VALUE</code> 的 <code>MEANING</code></td></tr>
    <tr><td>OA 回写装修等级失败</td><td>检查 <code>lovService.getValue</code> 反向翻译是否正常，确认 <code>MEANING</code> 能反查到 <code>VALUE</code></td></tr>
  </table>
</div>

<div class="kl-card">
  <div class="biz-kl-hdr">
    <span class="biz-tag" style="background:rgba(124,58,237,0.08);color:#7C3AED;border-color:rgba(124,58,237,0.18);"> 排查SQL</span>
    <h2>排查SQL汇总</h2>
    <p>装修等级值集常见排查SQL</p>
  </div>
...
<div class="kl-card">
  ...
  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:0 0 8px;">1. 进入值集配置页面</h4>
  <ol style="font-size:0.8rem;color:#374151;margin:0 0 16px;">
    <li>登录 HZERO 平台</li>
    <li>进入"开发管理" → "值集配置" → "装修等级"</li>
    <li>系统自动加载 LOV 编码为 <code>AE.FIXUP_GRADE</code> 的值项列表</li>
  </ol>

  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:0 0 8px;">2. 查询值项</h4>
  <ol style="font-size:0.8rem;color:#374151;margin:0 0 16px;">
    <li>在查询栏输入值编码、值名称或选择状态</li>
    <li>点击"查询"按钮，列表展示符合条件的值项</li>
    <li>不输入任何条件可查询全部值项</li>
  </ol>

  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:0 0 8px;">3. 新建值项</h4>
  <ol style="font-size:0.8rem;color:#374151;margin:0 0 16px;">
    <li>点击列表工具栏"新建"按钮</li>
    <li>在弹窗中填写：值编码（唯一）、值名称、描述、排序号、开始日期、结束日期</li>
    <li>点击"保存"按钮，系统校验必填项与编码唯一性后写入 <code>HPFM_LOV_VALUE</code> 表</li>
  </ol>

  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:0 0 8px;">4. 编辑值项</h4>
  <ol style="font-size:0.8rem;color:#374151;margin:0 0 16px;">
    <li>在列表中选择一条数据</li>
    <li>点击行操作"编辑"按钮</li>
    <li>在弹窗中修改值项信息（值编码通常不可编辑）</li>
    <li>点击"保存"按钮完成编辑</li>
  </ol>

  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:0 0 8px;">5. 删除值项</h4>
  <ol style="font-size:0.8rem;color:#374151;margin:0 0 16px;">
    <li>在列表中选择一条数据</li>
    <li>点击行操作"删除"按钮</li>
    <li>系统弹出确认框："确定删除该装修等级吗？"</li>
    <li>点击"确定"完成删除。若该值项被业务数据引用，系统将提示无法删除</li>
  </ol>

  <h4 style="font-size:0.88rem;font-weight:800;color:#1F2937;margin:0 0 8px;">6. 启用/禁用值项</h4>
  <ol style="font-size:0.8rem;color:#374151;margin:0 0 16px;">
    <li>在列表中选择一条数据</li>
    <li>点击行操作"启用/禁用"按钮</li>
    <li>系统切换 <code>ENABLE_FLAG</code> 字段值（1↔0）</li>
    <li>禁用后该值项不在业务下拉列表中展示</li>
  </ol>
  ...
  <div class="kl-tip" style="margin-top:14px;">
    ...
    <ul style="margin:6px 0 0;">
      <li>值编码在新增后通常不可修改，因为业务表中存储的是该编码</li>
      <li>排序号决定值项在下拉列表中的展示顺序，建议按业务习惯有序设置</li>
      <li>开始/结束日期控制值项的生效范围，过期或未生效的值项不在下拉列表中展示</li>
      <li>删除值项前请确认无业务数据引用，否则系统将阻止删除</li>
    </ul>
  </div>
</div>
