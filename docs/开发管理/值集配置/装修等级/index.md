...
<p>1. 打开新建值项弹窗/抽屉</p>
<p>2. 填写值编码、值名称、描述、排序号、开始/结束日期</p>
<p>3. 保存时校验值编码唯一性、必填项</p>
<p>4. 调用 HZERO 平台 LOV 服务新增接口写入 <code>HPFM_LOV_VALUE</code> 表</p>
<h4>按钮2：编辑（列表行操作）</h4>
<p><strong>触发条件</strong>：选择一条数据点击"编辑"</p>
<p><strong>执行逻辑</strong>：</p>
<p>1. 打开编辑值项弹窗/抽屉，回显当前值项数据</p>
<p>2. 修改值项信息（值编码通常不可编辑）</p>
<p>3. 保存时校验必填项</p>
<p>4. 调用 HZERO 平台 LOV 服务更新接口更新 <code>HPFM_LOV_VALUE</code> 表</p>
<h4>按钮3：删除（列表行操作）</h4>
<p><strong>触发条件</strong>：选择一条数据点击"删除"</p>
<p><strong>执行逻辑</strong>：</p>
<p>1. 弹出确认框："确定删除该装修等级吗？"</p>
<p>2. 确认后调用 HZERO 平台 LOV 服务删除接口</p>
<p>3. 检查该值项是否被业务数据引用，若被引用则提示无法删除</p>
<h4>按钮4：启用/禁用（列表行操作）</h4>
<p><strong>触发条件</strong>：选择一条数据点击"启用/禁用"</p>
<p><strong>执行逻辑</strong>：</p>
<p>1. 切换 <code>ENABLE_FLAG</code> 字段值（1↔0）</p>
<p>2. 调用 HZERO 平台 LOV 服务更新接口</p>
<p>3. 禁用后该值项不在业务下拉列表中展示</p>
</KbCard>
...
<KbCard title="保存校验">
<ul><li>校验1：值编码必填 —— 确保值项有唯一标识</li><li>校验2：值编码唯一 —— 同一 LOV 下值编码不可重复，避免业务引用歧义</li><li>校验3：值名称必填 —— 确保值项有展示名称</li><li>校验4：排序号必填 —— 确保值项在下拉列表中有明确顺序</li></ul>
</KbCard>
